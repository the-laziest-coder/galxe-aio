...
async def handle_x_migration(tls: TLSClient, headers):
    home_page = None
    migration_redirection_regex = re.compile(
        r"""(http(?:s)?://(?:www\.)?(twitter|x){1}\.com(/x)?/migrate([/?])?tok=[a-zA-Z0-9%\-_]+)+""", re.VERBOSE)
    response = await tls.get("https://x.com", headers=headers, raw=True)
    ...
    if migration_redirection_url:
        response = await tls.get(migration_redirection_url.group(0), headers=headers, raw=True)
        home_page = bs4.BeautifulSoup(response.content, 'lxml')
    ...
    if migration_form:
        ...
        response = await tls.request(method=method, url=url, data=request_payload, headers=headers, raw=True)
        home_page = bs4.BeautifulSoup(response.content, 'lxml')
    return home_page
