...
class ClientTransaction:
    ...
    def __init__(self):
        ...
        self.headers = {'Referer': 'https://x.com'}

    async def init(self, tls: TLSClient):
        try:
            home_page_response = await handle_x_migration(tls, self.headers)
            ...
            raise Exception(f'Init client transactions failed: {e}')

    async def get_indices(self, home_page_response, tls: TLSClient):
        key_byte_indices = []
        ...
        if on_demand_file:
            ...
            on_demand_file_response = await tls.get(on_demand_file_url, headers=self.headers, raw=True)
    ...
    def validate_response(self, response: Union[bs4.BeautifulSoup, CurlResponse]):
        if not isinstance(response, (bs4.BeautifulSoup, CurlResponse)):
    ...
    def generate_transaction_id(self, method: str, path: str, response=None, key=None, animation_key=None, time_now=None):
        try:
            ...
            raise Exception(f'Generate Client-Transaction-Id failed: {e}')
